...
class PatientsModel(models.Model):
    patient_id = models.CharField(max_length=7,default="", unique=True, blank=True, primary_key=True)
...
def generate_patient_id(sender, instance, **kwargs):
    if not instance.patient_id:
        ...
        new_patient_id = 1 if not last_patient else int(last_patient.patient_id[3:]) + 1
...
class DoctorsModel(models.Model):
    doctors_id = models.CharField(primary_key=True, null=False, max_length=6, unique=True, blank=True)
    fullname = models.CharField(max_length=100)
...
def generate_doctors_id(sender, instance, **kwargs):
    if not instance.doctors_id:
        ...
        new_doctor_id = 1 if not last_doctor else int(last_doctor.doctors_id[3:]) + 1
